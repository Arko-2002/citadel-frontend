...
const Save = ({ postId }) => {
  ...
  const handleSave = async (type) => {
    try {
      ...
      const user = await axios.post(
        `${import.meta.env.VITE_BASE_API}/api/user/${type}`,
        { postId: postId },
  ...
  return (
    <>
      ...
      >
        <img src={SaveOutline} alt="Save Button" className=" p-1  w-[24px]" />
      ...
      >
        <img src={SaveFilled} alt="Save Button" className=" p-1  w-[24px]" />
