...
function Post() {
    ...
    const post = async () => {
      try {
        ...
        const result = await axios.get(
          `${import.meta.env.VITE_BASE_API}/api/post/${postId}`
        );
        setPost(() => result.data);
        setStatus(() => 'fulfilled');
      } catch (err) {
  ...
  useEffect(() => {
    const getComments = async () => {
      try {
        const result = await axios.get(
          `${import.meta.env.VITE_BASE_API}/api/comment/${postId}`
        );
  ...
  const handleAddComment = async () => {
    try {
      ...
      const result = await axios.post(
        `${import.meta.env.VITE_BASE_API}/api/comment`,
        {
          comment: commentData,
          postId: postId,
        },
            ...
            <div className="max-w-5xl mx-auto pt-20 flex flex-col items-start gap-2 p-2 sm:gap-3">
              <div className="flex  items-center gap-2 w-full">
                ...
                <div className="flex text-[0.5rem] items-center gap-1">
                  ...
                  <span className="text-gray-600 opacity-60">Posted By</span>
              ...
              <h1 className="font-extrabold text-lg">{`${post.title}`}</h1>
              <Link to={`/subcategory/${post.subCategory}`}>
                <SubcateoryButton name={post.subCategory} small={true} />
              </Link>
              ...
              <div>{fetchComment ? fetchComment.length : 0} Comments</div>
              {userInfo?._id && (
                <Spin
                ...
                >
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={commentData ? false : true}
