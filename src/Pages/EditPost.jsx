    ...
    const getData = async () => {
      try {
        const result = await Promise.allSettled([
          ...
          axios.get(`${import.meta.env.VITE_BASE_API}/api/post/${editPostId}`),
        ]);
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const result = await axios.put(
        `${import.meta.env.VITE_BASE_API}/api/post`,
        {
          ...formData,
          community: communityData._id,
          subCategory: subCategoryData?.type ? subCategoryData?.type : 'Others',
          user: userData._id,
          postId: postData._id,
        },
        {
        ...
        <div className="max-w-5xl bg-white mx-auto w-full flex flex-col gap-2 px-2 sm:p-8">
            ...
            <span>
              {subCategoryData.type && (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <Link to={`/subcategory/${subCategoryData.type}`}>
                    <SubcateoryButton name={subCategoryData.type} />
                  </Link>
                </div>
              )}
            </span>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <button className="flex justify-start items-center shadow gap-2 p-2 bg-red-500 rounded-md w-fit hover:bg-red-600 text-white">
              <span className="text-xs font-bold">Upload a Photo/Video</span>
              <img src={ImageUpload} className="w-[16px] invert" />
            </button>
            <div className="p-2">
              <textarea
                type="text"
            ...
            <button className="mt-auto flex justify-start items-center self-end gap-2 p-2 bg-red-500 rounded w-fit hover:bg-red-600 text-white font-extrabold">
              <span className="text-xs font-bold text-white">Update Post</span>
            </button>
          </form>
        </div>
      </div>
