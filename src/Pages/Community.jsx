...
function Community({ setRender }) {
  ...
  const handleJoin = async () => {
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_API}/api/user/join`,
        {
  ...
  const handleLeave = async () => {
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_API}/api/user/leave`,
        {
          communityID: community._id,
        },
        {
    ...
    const getCommuinty = async () => {
      try {
        const result = await axios.get(
          `${import.meta.env.VITE_BASE_API}/api/community/${rid}`
        );
        if (result.data) {
          const postData = await axios.get(
            `${import.meta.env.VITE_BASE_API}/api/post/c/${result.data._id}`
          );
